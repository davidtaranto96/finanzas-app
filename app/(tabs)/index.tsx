...
export default function HomeScreen() {
  ...
  const monthTotals = useMemo(() => {
    ...
    const result = { income: 0, expense: 0, saving: 0, investment: 0 };
    for (const t of transactions) {
      const d = new Date(t.date);
      if (d.getFullYear() === year && d.getMonth() + 1 === month) {
        if (t.type === 'income') result.income += t.amount;
        else if (t.type === 'expense') result.expense += t.amount;
        else if (t.type === 'saving') result.saving += t.amount;
        else if (t.type === 'investment') result.investment += t.amount;
      }
    }
    return result;
  }, [transactions]);
